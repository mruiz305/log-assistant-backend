...
function makeLabel(kind, a, b, lang = 'es') {
  const es = lang === 'es';
  switch (kind) {
    case 'today': return es ? 'hoy' : 'today';
    case 'this_week': return es ? 'esta semana' : 'this week';
    case 'this_month': return es ? 'este mes' : 'this month';
    case 'last_month': return es ? 'último mes' : 'last month';
    case 'current_year': return es ? 'año actual' : 'current year';
    case 'last_year': return es ? 'año pasado' : 'last year';
    case 'year': return es ? `año ${a}` : `year ${a}`;
    case 'month_year': return `${a} ${b}`; // a=nombre mes, b=año
    case 'month_only': return es ? `${a} (más reciente)` : `${a} (most recent)`;
    case 'range': return es ? `rango ${a} a ${b}` : `range ${a} to ${b}`;
...
//  let label = makeLabel('default', 90, null, lang);
    ...
    let where = '';
    let label = lang === 'es' ? 'sin filtro de tiempo' : 'no time filter';
    let matched = false;
...
function buildKpiPackSql(message, opts = {}) {
  ...
  const w = extractTimeWindow(message, lang);
